...
<div class="container main">
    ...
    <div><span>作业状态：</span><span id="state"></span></div>
    ...
    <div>
        <table class="table">
            ...
            <?php
            $result->free_result();
            $strSQL="select * from pratice where contestID='$contestID'";
            $re=$db->GetData($strSQL);
            $strSQL="select * from problem where proID in (select proID from pratice where contestID='$contestID')";
            $result=$db->GetData($strSQL);
            while($problem=$result->fetch_assoc()){
                $con=$re->fetch_assoc();
                echo "<tr>
                        <td>{$con['problemOrder']}</td>
