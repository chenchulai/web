      ...
      *****************/
		...
		****************************/
		...
		_openFilter: function(header) {
			...
			var filterDiv = this._createDialogDiv("divXlsFilter", "筛选 - " + header.text(), filterContent);
			this._setupFilter();
		...
		_getFilterContent: function(header) {
			...
			filter = filter + '<div>&nbsp;搜索关键字：<input type="text" id="filterSearch" style="width: 200px;"></div>';
			filter = filter + '<div id="xlsFilterAllNone"><span id="xlsFilterAll">全选</span> | <span id="xlsFilterNone">反选</span></div>';
